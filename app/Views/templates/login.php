...
<body style="margin-bottom:100px;">
    ...
    <div class="container">
        <nav class="navbar navbar-expand-lg bg-light">
            <div class="container-fluid">
                <a class="navbar-brand" href="<?= base_url(); ?>/dashboard">KPU</a>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link <?= ($judul == 'Dashboard') ? 'active' : ''; ?>" aria-current="page" href="<?= base_url('dashboard'); ?>">Home</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= ($judul == 'Candidates') ? 'active' : ''; ?>" aria-current="page" href="<?= base_url(); ?>/dashboard/calon">Candidates</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= ($judul == 'Statistik') ? 'active' : ''; ?>" target="_blank" aria-current="page" href="<?= base_url(); ?>/dashboard/statistik">Statistik</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= ($judul == 'Kategori Pemilih') ? 'active' : ''; ?>" aria-current="page" href="<?= base_url(); ?>/dashboard/kategori">Kategori Pemilih</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" target="_blank" aria-current="page" href="<?= base_url(); ?>/dashboard/putra">Putra</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" target="_blank" aria-current="page" href="<?= base_url(); ?>/dashboard/putri">Putri</a>
                        </li>

                    </ul>
                    <div class="d-flex">
                        <div class="me-2">
                            <span><?= session('nama'); ?>/<?= session('username'); ?></span>
                        </div>
                        <div>
                            <a href="<?= base_url(); ?>/login/logout" style="text-decoration:none;">Logout</a>
    ...
    <script>
        ...
        CKEDITOR.replace('riwayat_capres');
        CKEDITOR.replace('riwayat_cawapres');
        ...
        $(document).on('keyup', '.select', function(e) {
            ...
            post(url, {
                ...
                .then(res => {
                    console.log(res);
                    let html = '';
                    html += '<a href="#" data-tahun="' + tahun + '" data-kategori="' + (order == 'cariabsen' ? order : kategori) + '" data-gender="' + gender + '" class="list-group-item list-group-item-action clear text-danger text-center"><i class="fa fa-trash-o"></i> Hapus Pencarian</a>';
                    if (res.data.length === 0) {
                        html += '<a href="#" class="list-group-item list-group-item-action insert">Data tidak ditemukan!.</a>';

                    } else {

                        for (let i = 0; i < res.data.length; i++) {
                            html += '<a href="#" data-tahun="' + tahun + '" data-kategori="' + (order == 'cariabsen' ? order : kategori) + '" data-gender="' + gender + '" data-username="' + res.data[i].username + '" class="list-group-item list-group-item-action insert">' + res.data[i].nama + '</a>';
                        }
                    }

                    $('.body' + tahun + (order == 'cariabsen' ? order : kategori) + gender).html(html);
                    $('.body' + tahun + (order == 'cariabsen' ? order : kategori) + gender).show();
                })
        ...
        $(document).on('click', '.insert', function(e) {
            ...
            $('.submit' + tahun + kategori + gender).val(username);
            $('.body' + tahun + kategori + gender).hide();
        ...
        $(document).on('click', '.clear', function(e) {
            ...
            $('.submit' + tahun + kategori + gender).val('');
            $('.body' + tahun + kategori + gender).hide();
        ...
        $(document).on('change', '.kategori', function(e) {
            ...
            post('dashboard/absen', {
                ...
                .then(res => {
                    ...
                    for (let i = 0; i < res.data.length; i++) {
                        html += '<a href="#" data-username="' + res.data[i].username + '" data-gender="' + gender + '" data-order="klikinsertabsen"  data-id="' + id + '" data-username="' + res.data[i].username + '" class="list-group-item list-group-item-action insertabsen">' + res.data[i].nama + '</a>';
                    }
                    $('.antrian' + gender).html(html);

                })

        })

        $(document).on('click', '.insertabsen', function(e) {
            e.preventDefault();
            let username = $(this).data('username');
            let gender = $(this).data('gender');
            let order = $(this).data('order');
            ...
            post('dashboard/absen', {
                ...
                .then(res => {
                    ...
                    if (res.data == false) {
                        ...
                        // html += '<div class="fw-bold">' + res.data.nama + '</div>';
                        html += 'Data di absen masih kosong!';
                        html += '</div>';
                        // html += '<span class="badge bg-primary rounded-pill">' + res.data.username + '</span>';
                        html += '</li>';
                    } else {
                        ...
                        html += '<div class="ms-2 me-auto">';
                        html += '<div class="fw-bold">' + res.data.nama + '</div>';
                        html += res.data.sub + '/' + res.data.kecamatan;
                        html += '</div>';
                        html += '<span class="badge bg-primary rounded-pill">' + res.data.username + '</span>';
                        html += '</li>';
                    }
                    $('.absen' + gender).html(html);
